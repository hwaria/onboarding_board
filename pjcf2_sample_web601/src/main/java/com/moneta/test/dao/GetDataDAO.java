...
@Repository("com.moneta.test.dao.ConnectDAO") 
public class GetDataDAO extends PaxAbstractMapper{
	...
	public List<CmtVO> viewComment(CmtVO cVO) {
		...
		return getSqlSession().selectList("com.moneta.test.mapper.newMapper.viewComment", cVO);
	}
	
	//댓글 달기 
	public int createComment(CmtVO cVO) {
		int createdCMT = getSqlSession().insert("com.moneta.test.mapper.newMapper.createComment", cVO);
	...
	public int commentNum(CmtVO cVO) {
		int cNum = getSqlSession().selectOne("com.moneta.test.mapper.newMapper.commentNum", cVO);
		return cNum;
	}
	...
	public List<CmtVO> getSelectedCMT(PagingService pg) {
		...
		return getSqlSession().selectList("com.moneta.test.mapper.newMapper.selectCmtList", pg);
	}
