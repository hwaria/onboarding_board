...
<c:if test="${listType eq 'searches'}"> 
	<button type="button" onclick="location.href='/main6'">전체 게시판으로 돌아가기</button>
</c:if>
...
<c:forEach items="${boardList}" var= "TestVO">
	<tr>
		...
		<td>
			<a href='/view?boardSeq=${TestVO.boardSeq}'>
			${TestVO.boardTitle}
			</a>
		</td>
		<td>${TestVO.nickNm}</td>
...
<!--페이지 넘버  -->
<!--현재 페이지는 두껍게 표시  -->
<c:forEach var="page" begin="${paging.startPage}" end="${paging.endPage}"> <!--현재 블럭의 페이지  -->
	<c:choose> 
		<c:when test ="${paging.curPage eq page}"> <!--현재 페이지가 클릭한 페이지와 같으면  -->
...
<c:if test="${paging.startPage + paging.pagesPerBlock <= paging.pagesNum}"> 
	<a href='/main6?page=${paging.startPage + paging.pagesPerBlock}'>&gt;</a>
</c:if>
